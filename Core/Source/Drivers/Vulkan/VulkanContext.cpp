...
namespace Dodo {
    ...
    static VkPresentModeKHR ConvertToVkPresentMode(VSyncMode vsyncMode)
    {
        switch (vsyncMode)
        {
            ...
            default                : ASSERT(false, "VSyncMode not supported!");
        }
    ...
    {
        VERIFY(CheckDriverVersionSupport(m_Instance.ApiVersion));
        ...
        // Convert enabled extensions to be Vulkan friendly.
    ...
    void VulkanContext::FindQueues()
    {
        ...
        ASSERT(m_GraphicsQueue.FamilyIndex.has_value(), "GraphicsQueue not found!");
    }
    ...
    void VulkanContext::InitDevice()
    {
        ...
        queueCreateInfo.queueFamilyIndex = m_GraphicsQueue.FamilyIndex.value();
        ...
        // Convert enabled extensions to be Vulkan friendly.
        std::vector<const char*> enabledExtensions{};
        for (const auto& extension : m_Device.EnabledExtensions)
        {
            enabledExtensions.push_back(extension.c_str());
        }
        ...
        // Get queue(s).
        vkGetDeviceQueue(m_Device.Handle, m_GraphicsQueue.FamilyIndex.value(), 0, &m_GraphicsQueue.Handle);
    }

    void VulkanContext::DisposeDevice() const
    {
    ...
    void VulkanContext::RecreateSwapchain()
    {
        ...
        if (m_GraphicsQueue.FamilyIndex != m_Swapchain.PresentQueue.FamilyIndex)
        {
            imageSharingMode         = VK_SHARING_MODE_CONCURRENT;
            queueFamilyIndices.at(0) = m_GraphicsQueue.FamilyIndex.value();
            queueFamilyIndices.at(1) = m_Swapchain.PresentQueue.FamilyIndex.value();
        }
        ...
        cmdPoolInfo.queueFamilyIndex = m_GraphicsQueue.FamilyIndex.value();
        ...
        // Create semaphores.
        if (!m_Swapchain.ImageAvailableSemaphores.empty() && !m_Swapchain.RenderCompleteSemaphores.empty())
        {
            for (size_t i = 0; i < backBufferCount; i++)
            {
                vkDestroySemaphore(m_Device.Handle, m_Swapchain.ImageAvailableSemaphores.at(i), nullptr);
                vkDestroySemaphore(m_Device.Handle, m_Swapchain.RenderCompleteSemaphores.at(i), nullptr);
            }
        }

        m_Swapchain.ImageAvailableSemaphores.clear();
        m_Swapchain.ImageAvailableSemaphores.resize(backBufferCount);

        m_Swapchain.RenderCompleteSemaphores.clear();
        m_Swapchain.RenderCompleteSemaphores.resize(backBufferCount);

        for (size_t i = 0; i < backBufferCount; i++)
        {
            VkSemaphoreCreateInfo semaphoreInfo{};
            semaphoreInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
            VERIFY_VK_RESULT(vkCreateSemaphore(m_Device.Handle, &semaphoreInfo, nullptr, &m_Swapchain.ImageAvailableSemaphores.at(i)));
            VERIFY_VK_RESULT(vkCreateSemaphore(m_Device.Handle, &semaphoreInfo, nullptr, &m_Swapchain.RenderCompleteSemaphores.at(i)));
        }

        // Create wait fences.
        if (!m_Swapchain.WaitFences.empty())
        {
            for (auto& waitFence : m_Swapchain.WaitFences)
            {
                vkDestroyFence(m_Device.Handle, waitFence, nullptr);
            }
        }
    ...
    {
        return !m_Swapchain.SurfaceFormats.empty() && !m_Swapchain.PresentModes.empty();
    }
    ...
    {
        VkPresentModeKHR targetMode = ConvertToVkPresentMode(Renderer::GetSettings().VSyncMode);
    ...
    void VulkanContext::FindPresentQueue()
    {
        // Get available queues.
        uint32_t queueCount = 0;
        vkGetPhysicalDeviceQueueFamilyProperties(m_Adapter.Handle, &queueCount, nullptr);
        ...
        for (const auto& queue : queues)
        {
            DODO_MAYBE_UNUSED(queue);
            VkBool32 supportsPresent = false;
            m_Funcs.pfnGetPhysicalDeviceSurfaceSupportKHR(m_Adapter.Handle, queueIndex, m_Surface.Handle, &supportsPresent);
            if (supportsPresent)
            {
                m_Swapchain.PresentQueue.FamilyIndex = queueIndex;
                break;
            }
        ...
        // Get present queue.
        ASSERT(m_Swapchain.PresentQueue.FamilyIndex.has_value(), "PresentQueue not found!");
        vkGetDeviceQueue(m_Device.Handle, m_Swapchain.PresentQueue.FamilyIndex.value(), 0, &m_Swapchain.PresentQueue.Handle);
    }

    void VulkanContext::DisposeSwapchain() const
    {
